...
13. **Softmax 函数**：模型的输出是一个 CC-维的向量，表示每个类别的得分。通过 **softmax 函数** 将得分转换成一个概率分布，确保所有类别的概率和为 1。Softmax 函数的公式为：
    
    pi=ezi∑j=1Cezjp_i = \frac{e^{z_i}}{\sum_{j=1}^{C} e^{z_j}}
    
    其中，ziz_i 是模型对类别 ii 的得分（logits）。
    
14. **计算损失**：然后，利用多分类交叉熵损失函数来计算损失。
...
20. **初始化**：使用目标值的均值作为初始模型 \( F_0(x) \)。
21. **计算残差**：计算当前模型的残差 \( r_i^{(t)} = y_i - F_{t-1}(x_i) \)。
22. **训练决策树**：训练一个新的决策树 \( h_t(x) \)，使得它拟合残差。
23. **更新模型**：将树的输出加权更新到模型中 \( F_t(x) = F_{t-1}(x) + \alpha_t h_t(x) \)。
24. **重复步骤 2-4**，直到满足停止条件。

### **GBDT的优缺点**

#### **优点**：
25. **高准确性**：GBDT在很多实际问题中都表现出色，能够很好地拟合复杂的非线性数据。
26. **灵活性强**：可以处理各种类型的数据，包括分类和回归问题。
27. **强大的泛化能力**：通过逐步减小残差，GBDT能够有效减少模型的偏差，并且对于许多复杂问题有很好的拟合效果。

#### **缺点**：
28. **计算开销大**：训练过程是逐步的，需要每一轮都训练一棵新的树，因此训练时间较长，尤其是数据集较大时。
29. **容易过拟合**：尽管GBDT具有强大的拟合能力，但如果树的深度过大或者迭代次数过多，可能会导致过拟合。
30. **对噪声敏感**：GBDT对数据中的噪声较为敏感，特别是当数据集中的噪声较多时，GBDT可能会在训练中过于拟合噪声。
...
31. **自助采样**：从原始训练数据集中有放回地随机抽取多个子数据集。每个子数据集的大小通常与原始数据集相同。
32. **独立训练**：对每个子数据集训练一个模型。训练过程是相互独立的，且每个模型可能是不同类型的（通常是决策树）。
33. **集成预测**：对于分类问题，通常使用**投票**（多数投票）；对于回归问题，使用**平均值**来组合所有模型的预测结果。
...
42. **基于梯度提升树（GBDT）**：LightGBM属于梯度提升框架的一种，它通过构建多棵决策树，逐步优化模型。
43. **支持分类特征**：LightGBM可以直接处理类别特征，而不需要额外的预处理。
44. **支持大规模数据**：LightGBM设计时特别关注内存效率，能够处理大规模数据集，且在并行计算上做了大量优化。
45. **支持GPU加速**：LightGBM可以利用GPU进行训练，大大加快了模型训练速度。
46. **自适应叶子生长策略**：LightGBM采用了独特的叶子生长策略，而不是传统的深度限制策略，这使得它能够更好地捕捉数据中的复杂关系。
...
47. **正则化（Regularization）**：XGBoost引入了L1和L2正则化（类似于Lasso和Ridge回归），帮助防止模型过拟合。
48. **加速（Speedup）**：通过并行化计算、缓存优化和精简的计算过程，大大提高了训练速度。
49. **处理缺失值**：XGBoost能够自动处理数据中的缺失值，不需要额外的预处理。
50. **剪枝（Pruning）**：通过设置最大深度、最小样本分裂等参数来控制树的复杂度，避免过拟合。
51. **树的增量训练**：在构建每一棵树时，XGBoost并不是从头开始构建，而是根据之前的树进行增量训练。
...
52. **数据标准化**：对数据进行中心化，使其均值为0。
53. **计算协方差矩阵**：计算数据集的协方差矩阵 Σ\Sigma。
54. **特征值分解**：对协方差矩阵进行特征值分解，得到特征值和特征向量。
55. **选择主成分**：选择前 kk 个特征值最大的特征向量。
56. **投影数据**：将原始数据投影到这些主成分上，得到降维后的数据。
...
64. **输入层**：接收数据，比如图像的每个像素值、文本的单词向量等。
65. **隐藏层**：数据经过多个“隐藏层”进行处理，每一层都有许多神经元，它们通过权重进行连接并执行计算。
66. **输出层**：最终给出模型的预测结果，如图像分类中的标签、语音识别中的词语等。
...
67. **选择最佳特征进行划分**：根据某种准则（如信息增益、基尼指数等），选择最能有效划分数据的特征作为节点的判定条件。常见的选择标准有：
    
    - **信息增益（Information Gain）**：用于衡量某个特征划分数据集后，数据的不确定性减少了多少。信息增益越大，说明这个特征对于数据的划分越重要。
    - **基尼指数（Gini Index）**：衡量数据集纯度的一种指标，基尼指数越小，表示数据集的纯度越高。
68. **递归划分**：在每个节点，使用选择的特征对数据集进行划分，直到满足停止条件。
    
69. **停止条件**：决策树的构建会在某些条件下停止，常见的停止条件包括：
    
    - 所有数据属于同一类别。
    - 剩下的特征不能有效划分数据。
    - 达到最大树深或最小样本数等限制。
70. **构建叶子节点**：一旦满足停止条件，每个叶子节点就代表了最终的预测结果。分类问题中，叶子节点通常是类别标签；回归问题中，叶子节点通常是一个连续值。
...
71. 初始化模型的参数（比如权重）。
72. 计算损失函数，衡量当前模型的表现。
73. 计算梯度（损失函数相对于参数的导数）。
74. 根据梯度更新参数，使得损失减小。
75. 重复以上步骤，直到损失函数收敛（即损失值不再变化）。
...
76. **理解数据的基本情况**：了解数据的基本统计特征、结构和分布。
77. **发现数据中的问题**：如缺失值、异常值、重复值等。
78. **理解特征之间的关系**：发现特征之间可能的相关性，或者数据的分布模式。
79. **探索潜在的业务含义**：确定哪些特征可能影响目标变量。
...
80. **加载和查看数据**：了解数据的基本情况和结构。
81. **处理缺失值**：检查缺失数据并做适当处理。
82. **检查数据分布和异常值**：了解数据的分布，找出异常点。
83. **可视化分析**：通过可视化图表深入理解数据的特征和关系。
84. **特征工程**：做数据转换、特征选择等操作，为建模做准备。
85. **目标变量分析**：了解目标变量的分布，检查是否有偏态等。
...
86. **基于模型的特征重要性** 一些模型（如决策树、随机森林、XGBoost等）可以直接提供特征重要性。通过这些模型训练出的模型系数或其他内置方法来评估每个特征的重要性。
...
89. **Permutation Importance（置换重要性）**
...
90. **标准化数据**：计算每个特征的均值和标准差，对数据进行标准化。
91. **计算协方差矩阵**：计算标准化后的数据协方差矩阵，衡量特征之间的相关性。
92. **特征分解**：对协方差矩阵进行特征值分解，得到特征值和特征向量。
93. **选择主成分**：按特征值的大小降序排列，选择前几个特征向量作为主成分。
94. **转换数据**：将原始数据投影到选择的主成分上，得到降维后的数据。
...
95. 随机选择2个点作为簇中心（红色和蓝色的点）。
96. 根据每个点到这两个中心的距离，将数据分配到最近的中心。
97. 重新计算每个簇的中心点，即每个簇内所有点的均值。
98. 再次根据新的中心点分配数据点，重新计算中心点。
99. 重复这个过程直到簇中心不再发生变化。
...
100. **效率**：K-means是一个相对高效的算法，尤其适用于大规模数据集。它的时间复杂度为O(n_k_d)，其中n是数据点的数量，k是簇的数量，d是数据点的维度。
    
101. **可扩展性**：K-means算法对于数据的规模具有良好的扩展性。当数据量较大时，它仍然能保持较快的运行速度，尤其是在聚类数较小的情况下。
    
102. **灵活性**：K-means能够用于各种不同类型的任务，如客户细分、图像压缩、推荐系统等，但它更适合处理较为简单且分布较为均匀的数据集。
